...
df = pd.read_csv(r"C:\Users\hp\Desktop\hackathon\application_train.csv")  
type(df)
df.head()
df.info()
df.describe()
...
X = df.drop(columns=["TARGET", "SK_ID_CURR", "RISK_SCORE"], errors='ignore')
y = df["TARGET"]
...
artifacts = {
    'log_reg_model': log_reg_scaled,
    'scaler': scaler,
    'imputer': None,  # If you use an imputer, add it here
    'feature_names': list(X_train_filled.columns),
    'label_encoders': {},  # Add your encoders if you use them
    'rf_model': None,      # Add RandomForest if you train it
    'feature_importance': coef_df.rename(columns={'weight': 'importance'}),
}

joblib.dump(artifacts, 'loan_decision_artifacts.joblib')
print("Artifacts saved to loan_decision_artifacts.joblib")